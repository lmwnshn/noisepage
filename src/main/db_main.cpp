...
void DBMain::Run() {
  // TODO(WAN): When you can specify per-txn retention policies, this hacky enable/disable+sleep should go away.
  if (replication_manager_ != DISABLED) {
    replication_manager_->DisableReplication();
  }
  TryLoadStartupDDL();
  std::this_thread::sleep_for(std::chrono::seconds(5));
  if (replication_manager_ != DISABLED) {
    replication_manager_->EnableReplication();
  }
